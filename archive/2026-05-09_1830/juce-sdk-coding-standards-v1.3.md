...
7. **Generated or agent-written code MUST pass the same gates as human code.** Build, tests, static analysis, sanitiser runs, and VST3 validation are mandatory for release-candidate code under the validation profile in §13.1. PR and nightly subsets may be smaller, but they must be explicit and must not create a lower standard for agent-written changes.
...
Compiler baselines are time-sensitive. When this standard is revised, confirm the supported compiler versions against the official vendor/toolchain sources, then record the checked date and rationale in dependency or build metadata. Do not silently carry a stale minimum compiler version into a later product cycle.
...
> **Convention.** Examples in this document use the literal target name `MyPlugin`. In a real project, replace it with the target produced by `juce_add_plugin()`. CMake snippets MUST NOT reference an undeclared variable such as `${PROJECT_TARGET}`; an unset or misspelled target placeholder can fail configuration, misparse the command, or hide that options were never applied to the intended target.
...
void MyProcessor::readTransport() noexcept
{
    if (auto* ph = getPlayHead()) {
        if (auto pos = ph->getPosition()) {
            ...
            const bool   isPlaying  = pos->getIsPlaying().orFallback(false);
...
- Read it from non-audio lifecycle or message-thread code after the wrapper has been initialised. Do not make constructor-time decisions from `wrapperType`; early construction can observe `wrapperType_Undefined` or another transient value depending on the wrapper path.
- Do not branch on it inside `processBlock` for performance-relevant code paths.
...
if (gainParam_ == nullptr) {
    markProcessorInvalid("Missing required parameter: gain"); // project-defined, host-safe failure path
    return;
}
...
Do not allow an exception to escape a JUCE or host callback just to signal a missing parameter; convert it to a safe invalid/bypassed state and a non-real-time diagnostic.
...
`makeView(buffer, startSample, sampleCount)` MUST be non-allocating and must account for the sample offset by passing channel pointers advanced by `startSample`, or by using an equivalent DSP view type that carries the offset explicitly. Do not pass the full-block channel pointer with a shortened length unless the DSP core also receives and applies the start offset.
...
Each atomic load/store MUST carry a one-line comment at the use site naming which of those three categories applies. This makes review tractable and prevents drift when code is refactored.
...
- Missing required resources, parameters, or state discovered during plug-in construction or lifecycle callbacks MUST be converted to a safe invalid/bypassed state and a non-real-time diagnostic. Do not let an exception escape to the host as the failure mechanism.
...
JUCE-built VST3 release candidates MUST be run through both validators before shipping, unless a validator is unavailable for the release platform or runner. Any skipped validator requires a documented waiver that records the reason, affected platforms, risk, and compensating host tests.

PR and nightly validation levels are defined by the CI profiles in §13.1. At minimum, PR builds SHOULD run a fast baseline validator path where practical, and nightly/release-branch builds SHOULD run stricter stress levels.
...
| --- | --- | --- |
| PR | Every pull request or agent change batch | Configure, build, relevant unit/DSP/state tests, and fast static checks for the changed area. Include a baseline VST3 validation path where runner support and runtime allow it. |
| Nightly / mainline | Scheduled or post-merge | Full build matrix, all tests, static analysis, sanitiser jobs, VST3 validation, and stricter `pluginval` runs. Failures must be triaged, not ignored as advisory noise. |
| Release candidate | Any build considered for external testing, signing, notarisation, or shipment | All production gates below MUST pass, or an explicit waiver must record the unavailable tool, risk, owner, expiry, and compensating validation. |

A production release-candidate pipeline MUST include:
...
10. Run Tracktion `pluginval` at the project's release-candidate strictness level.
11. Package artefact.
12. Smoke-test in representative plug-in hosts where an automated or manual host path is available; record any skipped host coverage.
